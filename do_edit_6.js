...
async function main() {
  ...
  // Target X height is 122 (Real height of E and S). Original X height is 167 (from Y=49 to Y=215).
  let scale = 122 / 167; // ~0.7305
  let newH = Math.round(225 * scale); // 164
  let newW = Math.round(315 * scale); // 230
  ...
  // We need to make sure we erase the ENTIRE bottom X, but not E and S.
  // E ends at X=160. S starts at X=270.
  // We erase X=161 to X=269, Y=277 to 398.
  for (let y = 265; y <= 405; y++) {
    for (let x = 162; x <= 268; x++) {
      orig.setPixelColor(0x00000000, x, y);
    }
  }
  ...
  // X Top flat Y in orig is 49. In crop it's 49-30=19. Scaled is 19 * scale = 13.88 (14)
  // Target Y for top of E is 277.
  // startY + 14 = 277 -> startY = 263
  let startY = 277 - Math.round(19 * scale);
  
  // Gap center is 215. newW = 230.
  ...
  console.log('Done! logo.png has been updated to version 6.');
}
